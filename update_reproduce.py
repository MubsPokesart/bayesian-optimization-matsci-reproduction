...
N_ENSEMBLE = 50  # Number of independent runs (seeds)
N_INITIAL = 2
TOP_K_PERCENT = 0.05
...
N_RESTARTS_GP = 2  # Number of restarts for sklearn GP optimization (2-5 is good balance)
N_ESTIMATORS_RF = 100  # Number of trees in Random Forest
N_JOBS = 20  # Number of parallel jobs (set to number of CPU cores)
...
def build_random_forest_surrogate(X_train_norm, y_train_norm, seed):
    ...
    rf = RandomForestRegressor(
        ...
        bootstrap=True,
        random_state=seed,
        n_jobs=-1  # Use all available cores for RF tree building
    )
...
def _run_single_seed(seed, X_feature, y_objective, top_indices, n_total, surrogate_type):
    """Worker function to run a single seed (for parallelization)."""
    random.seed(seed)
    np.random.seed(seed)
...
def run_benchmark_generic(X_feature, y_objective, top_indices, n_total, seed_list, surrogate_type='GP-sklearn'):
    ...
    """
    print(f"Running {N_ENSEMBLE} seeds in parallel with {N_JOBS} jobs...")

    # Parallel execution over seeds
    results = Parallel(n_jobs=N_JOBS, verbose=10)(
        delayed(_run_single_seed)(
            seed, X_feature, y_objective, top_indices, n_total, surrogate_type
        )
